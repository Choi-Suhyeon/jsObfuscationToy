...
# 가정 : let은 키워드로 생각하겠음. 원래는 아님. 하지만, 그렇다고 let이 키워드처럼도 동작하고 어떨 때는 식별자로도 동작하면 내가 너무 힘듬.
# TODO : 구조분해도 고려해야 됨.
...
elems_of_kind:    tuple[str] = (
    ...
    'TemplateHeadLiteral'      , 'TemplateMiddleLiteral' , 'TemplateTailLiteral' , 'TemplateLiteral'    , 'Let'              , 
)
...
def analyze_lexeme(code: str) -> tuple[tuple[Kind, str]]:
    ...
    def analyze_lexeme_inner(code: str, context: dict[str: bool], result: tuple[tuple[Kind, str]]) -> tuple[tuple[Kind, str]]:
        ...
        if interim_kind is None:
            print('[ERR] UNKNOWN Substring :', code[:40])
            exit(1)
